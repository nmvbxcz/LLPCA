function [] = LLPCA(datasetName,number,d,k)  
    ...
    for s = 1:size(number,2)
        ...
        for index=1:1
            ...
            img_flatten = reshape(img,m*n,c);
            train_data = img_flatten(index_train,:);
            train_label = gt(index_train);
            test_data = img_flatten(index_test,:);
            test_label = gt(index_test);
            [ ~,~, ~, ~, decision_values,~ ] = mysvmc(train_data, train_label, img_flatten, reshape(gt,m*n,1), 0, 0, 2, 1);
            [~, topKLabels] = sort(decision_values, 2, 'descend');
            topKLabels = topKLabels(:, 1:k);

            row_sums = sum(decision_values, 2);
            normalized_A_sum = decision_values ./ repmat(row_sums, 1, size(decision_values, 2));
            normalized_A_max = max(normalized_A_sum,[],2)/3;
            prop_map = normalized_A_sum > normalized_A_max;
            for i = 1:m*n
                if sum(prop_map(i,:))<k
                    [~,max_k_ind] = sort(normalized_A_sum(i,:), 'descend');
                    max_k_ind = max_k_ind(1:k);
                    prop_map(i,max_k_ind) = 1;
                end
            end
            ...
            iter = 0;
            all_map = ones(size(label));
            all_map(:,2:end) = prop_map;

            while true
                iter = iter + 1
                label = W*(label.*all_map);  
                [~,P] = max(label,[],2);
                predict = P-1;
